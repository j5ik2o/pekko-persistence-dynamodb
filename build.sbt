...
lazy val test = (project in file("test"))
  ...
  .settings(
    ...
    libraryDependencies ++= Seq(
      ...
      amazonaws.dynamodb,
      javaDevJnv.jna,
      "com.github.j5ik2o" %% "docker-controller-scala-scalatest"      % "1.15.19",
      "com.github.j5ik2o" %% "docker-controller-scala-dynamodb-local" % "1.15.19"
    ),
    libraryDependencies ++= {
...
lazy val `base` = (project in file("base/base"))
  ...
  .settings(
    ...
    libraryDependencies ++= Seq(
      slf4j.api,
      iheart.ficus,
      pekko.slf4j(pekkoVersion),
      pekko.stream(pekkoVersion),
      "org.scala-lang.modules"               %% "scala-java8-compat" % "1.0.2",
      pekko.testkit(pekkoVersion)             % Test,
      logback.classic                         % Test,
      slf4j.julToSlf4J                        % Test,
      pekko.streamTestkit(pekkoVersion)       % Test,
...
lazy val `journal-base` = (project in file("journal/journal-base"))
  ...
  .settings(
    ...
    libraryDependencies ++= Seq(
      ...
      pekko.testkit(pekkoVersion)             % Test,
      pekko.streamTestkit(pekkoVersion)       % Test,
      pekko.persistenceTck(pekkoVersion)      % Test,
      scalatest.scalatest(scalaTest32Version) % Test,
